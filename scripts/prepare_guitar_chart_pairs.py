#!/usr/bin/env python3
"""Prepare local Clone Hero ``notes.mid`` files for chart-pair fine-tuning.

This offline bridge extracts the five standard lanes from ``PART GUITAR`` and
writes the ``strum-chart-pairs/v1`` JSONL consumed by
``train_chart_transform.py``.  It intentionally excludes open notes and
non-lane modifiers: the first chart-transform baseline predicts five lanes.
"""
...
def discover_notes_mid(inputs: Iterable[Path], list_files: Iterable[Path] = ()) -> list[Path]:
    ...
    for candidate in candidates:
        if candidate.is_dir():
            discovered.update(path.resolve() for path in candidate.rglob("notes.mid") if path.is_file())
...
def parse_guitar_difficulties(midi_path: Path) -> dict[str, list[dict[str, object]]]:
    """Extract grouped PART GUITAR lane events for all standard difficulties."""
    midi = mido.MidiFile(midi_path)
    guitar_track = next((track for track in midi.tracks if track.name == "PART GUITAR"), None)
    if guitar_track is None:
        raise PreparationError(f"PART GUITAR track not found: {midi_path}")
    ...
    return {
        difficulty: [
            {"time_ms": round(_tick_to_ms(tick, midi.ticks_per_beat, tempo_map), 3), "lanes": sorted(lanes)}
...
def prepare_dataset(
    midi_paths: Iterable[Path], output_dir: Path, options: PreparationOptions, *, overwrite: bool = False
) -> dict[str, object]:
    ...
    if not records:
        ...
        raise PreparationError(f"no usable PART GUITAR Expert charts: {details}")
    ...
    if not overwrite and (records_path.exists() or manifest_path.exists()):
        raise PreparationError(f"output already contains {records_path.name} or {manifest_path.name}; pass --overwrite")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare local PART GUITAR chart pairs from notes.mid files.")
    parser.add_argument("--input", nargs="*", type=Path, default=[], help="notes.mid file(s) or directories to scan")
    parser.add_argument("--list-file", action="append", type=Path, default=[], help="UTF-8 list of notes.mid files/directories")
    parser.add_argument("--output-dir", type=Path, required=True, help="directory for pairs.jsonl and dataset-manifest.json")
    parser.add_argument("--target-difficulty", choices=["Hard", "Medium", "Easy"], required=True)
    parser.add_argument("--dataset-id", required=True)
    parser.add_argument("--provenance", required=True, help="source and collection method for authorized charts")
    parser.add_argument("--license", required=True, help="license/permission covering model-training use")
    parser.add_argument("--overwrite", action="store_true", help="replace existing pairs.jsonl/dataset-manifest.json")
...
def main() -> None:
    ...
    print(json.dumps({"manifest_path": str(result["manifest_path"]), "record_count": result["record_count"], "skipped": result["skipped"]}, indent=2))
